...
The showcase file should render all component variations in a grid layout:
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {{/* Render each variation with a title and example */}}
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-gray-700">Default</h3>
          <div className="p-4 bg-white rounded-lg border">
            <{component_name}>Example Content</{component_name}>
          </div>
        </div>
        {{/* Add more variations based on component props */}}
      </div>
    </div>
  );
}}
